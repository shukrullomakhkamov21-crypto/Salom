...
# --- 5. SHARED LOGIC ---
RESERVED_BUTTONS = ["Test yechish 📝", "So'zlar ombori 📚", "📢 Rassilka", "📊 Statistika", 
...
async def test_start(m: types.Message, state: FSMContext):
    await state.clear()
    is_new = await db.execute("SELECT user_id FROM test_users WHERE user_id=?", (m.from_user.id,), fetch=True)
    if not is_new:
        await db.execute("INSERT OR IGNORE INTO test_users (user_id, name) VALUES (?, ?)", (m.from_user.id, m.from_user.full_name))
        for admin in ADMIN_LIST:
            try: await bot_t.send_message(admin, f"🆕 Yangi foydalanuvchi:\nIsm: {m.from_user.full_name}\nID: {m.from_user.id}")
            except: pass
...
async def take_test(m: types.Message):
    ...
    if not tests: return await m.answer("🎉 Hamma testlarni yechib bo'ldingiz, tabriklayman!")
...
async def handle_poll_answer(quiz: types.PollAnswer):
    ...
    # Keyingi testni chaqirish uchun "fake" xabar obyekti
    fake_msg = types.Message(message_id=0, date=None, chat=types.Chat(id=data['user_id'], type="private"), 
                             from_user=types.User(id=data['user_id'], is_bot=False, first_name="User"), text="Test yechish 📝")
    await take_test(fake_msg)

@dp_t.message(F.text == "📊 Statistika", IsAdmin())
async def t_stat(m: types.Message):
    ...
    total_tests = await db.execute("SELECT COUNT(*) as c FROM tests", fetch=True)
    
    txt = f"📊 <b>Statistika:</b>\nJami testlar: {total_tests[0]['c']}\n\n<b>Reyting:</b>\n"
    for i, u in enumerate(users, 1):
        txt += f"{i}. {u['name']} - {u['score']} ta ✅\n"
    await m.answer(txt, parse_mode="HTML")

@dp_t.message(F.text == "🗑 Test o'chirish", IsAdmin())
async def del_test_list(m: types.Message, state: FSMContext):
    tests = await db.execute("SELECT * FROM tests", fetch=True)
    if not tests: return await m.answer("Testlar yo'q.")
    builder = InlineKeyboardBuilder()
    for t in tests:
        builder.button(text=f"{t['q'][:20]}...", callback_data=f"del_t_{t['id']}")
    builder.adjust(1)
    await m.answer("O'chirish uchun testni tanlang:", reply_markup=builder.as_markup())

@dp_t.callback_query(F.data.startswith("del_t_"))
async def process_del_test(c: types.CallbackQuery):
    t_id = c.data.split("_")[2]
    await db.execute("DELETE FROM tests WHERE id=?", (t_id,))
    await db.execute("DELETE FROM solved WHERE test_id=?", (t_id,))
    await c.message.edit_text("✅ Test o'chirildi!")
...
async def word_menu(m: types.Message, state: FSMContext):
    ...
    await m.answer("Siz bu yerga yod olgan so'zlaringizni yozasiz, men esa eslab qolaman!", reply_markup=word_menu_kb)
...
async def w_cnt(m: types.Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="So'zlarni ko'rish 👀", callback_data="view_words")]])
    await m.answer(f"So'zlaringiz: {c[0]['c']} ta", reply_markup=kb)

@dp_t.callback_query(F.data == "view_words")
async def view_words(c: types.CallbackQuery):
    words = await db.execute("SELECT word FROM words WHERE user_id=?", (c.from_user.id,), fetch=True)
    if not words: return await c.answer("So'zlar hali yo'q")
    txt = "📚 <b>Sizning so'zlaringiz:</b>\n\n" + "\n".join([f"🔹 {w['word']}" for w in words])
    await c.message.answer(txt, parse_mode="HTML")
    await c.answer()

@dp_t.message(WordStates.main, F.text == "Kerakli so'zni o'chirish")
async def del_word_inline(m: types.Message):
    ...
    builder.adjust(2)
    await m.answer("O'chirish uchun so'zni tanlang:", reply_markup=builder.as_markup())

@dp_t.callback_query(F.data.startswith("dw_"))
async def confirm_del_word(c: types.CallbackQuery):
    w_id = c.data.split("_")[1]
    await db.execute("DELETE FROM words WHERE id=?", (w_id,))
    await c.message.edit_text("🗑 So'z o'chirildi.")
...
async def s_st(m: types.Message):
    is_new = await db.execute("SELECT user_id FROM shop_users WHERE user_id=?", (m.from_user.id,), fetch=True)
    if not is_new:
        await db.execute("INSERT OR IGNORE INTO shop_users (user_id, name) VALUES (?, ?)", (m.from_user.id, m.from_user.full_name))
        for admin in ADMIN_LIST:
            try: await bot_s.send_message(admin, f"🆕 Shop Bot yangi foydalanuvchi:\nIsm: {m.from_user.full_name}\nID: {m.from_user.id}")
            except: pass
...
async def s_add_st(m: types.Message, state: FSMContext):
    await m.answer("Guruh linkini yuboring (masalan @sevishganlar_guruhi):"); await state.set_state(ShopStates.link)

@dp_s.message(ShopStates.link)
async def s_add_l(m: types.Message, state: FSMContext):
    # Linkni o'z holicha saqlash (Markdown/HTML xatolarini oldini olish uchun text formatida)
    await state.update_data(link=m.text); await m.answer("Narxni yuboring:"); await state.set_state(ShopStates.price)
...
async def s_add_p(m: types.Message, state: FSMContext):
    ...
    await m.answer("✅ Guruh va narx saqlandi!"); await state.clear()
...
async def s_stat(m: types.Message):
    ...
    txt = f"📊 <b>Shop Bot Statistika:</b>\nJami foydalanuvchilar: {len(users)} ta\n\n<b>Foydalanuvchilar ro'yxati:</b>\n"
    for u in users[:50]: # Ro'yxat juda uzun bo'lib ketmasligi uchun 50ta
        txt += f"• {u['name']} (ID: {u['user_id']})\n"
    await m.answer(txt, parse_mode="HTML")

# --- RASSILKA (SHOP) ---
@dp_s.message(F.text == "📢 Rassilka", IsAdmin())
async def s_bc_st(m: types.Message, state: FSMContext):
    await m.answer("Rassilka xabarini yuboring:"); await state.set_state(ShopStates.bc)
...
async def s_bc_send(m: types.Message, state: FSMContext):
    ...
    await m.answer(f"✅ Xabar {c} ta foydalanuvchiga yuborildi."); await state.clear()

# ==========================================
#         CRON TASKS (AVTOMATIK XABARLAR)
# ==========================================
@aiocron.crontab('30 19 * * *') # Har kuni 19:30
async def evening_ranking():
    users = await db.execute("SELECT u.name, (SELECT COUNT(*) FROM solved WHERE user_id=u.user_id) as score FROM test_users u ORDER BY score DESC LIMIT 10", fetch=True)
    if not users: return
    txt = "🏆 <b>Bugungi TOP 10 bilimdonlar:</b>\n\n"
    for i, u in enumerate(users, 1):
        txt += f"{i}. {u['name']} — {u['score']} ta ✅\n"
    
    all_users = await db.execute("SELECT user_id FROM test_users", fetch=True)
    for au in all_users:
        try: await bot_t.send_message(au['user_id'], txt, parse_mode="HTML")
        except: pass

@aiocron.crontab('00 06 * * *') # Har kuni 06:00
async def morning_wake():
    all_users = await db.execute("SELECT user_id FROM test_users", fetch=True)
    for au in all_users:
        try: await bot_t.send_message(au['user_id'], "☀️ Diqqat diqqat! Botimiz ishlayapti, uyg'oning!")
        except: pass

# --- TEST QO'SHISH VA RASSILKA (TEST BOT) ---
...
async def add_t_v3(m: types.Message, state: FSMContext):
    ...
    await m.answer("✅ Test saqlandi!"); await state.clear()

# --- OLD PREVIOUS HANDLERS (RASSILKA TEST) ---
